...
var ViewsCacher = /** @class */ (function (_super) {
    ...
    ViewsCacher.getDerivedStateFromProps = function (nextProps, prevState) {
        ...
        if (displayedViewInfo) {
            ...
            if (shownInfo_1) {
                var $local = shownInfo_1.localRunContext.$local, _b = shownInfo_1.routingPart, params = _b.params, config = _b.config;
    ...
    ViewsCacher.prototype.updateCachedInfos = function () {
        var _a = this.props, cachedViewsInfo = _a.cachedViewsInfo, updateAppState = _a.updateAppState, renderedInfos = this.state.items.map(function (item) { return item.viewInfo; }), missingInfos = renderedInfos.filter(function (renderedInfo) { return !cachedViewsInfo[renderedInfo.routingPart.config.id]; });
        if (missingInfos.length) {
            ...
            updateAppState({ cachedViewsInfo: cachedViewsInfo });
        }
    };
    ViewsCacher.prototype.clearCachedInfos = function () {
        this.props.updateAppState({ cachedViewsInfo: {} });
    };
    return ViewsCacher;
}(React.Component));
export { ViewsCacher };
