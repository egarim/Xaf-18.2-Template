...
import { compileSetter as dxCompileSetter } from "devextreme/utils";
...
export function compileGetter(expr, fullExpr, src) {
    ...
    if (expr.charAt(0) === "!") {
        var getter_2 = compileGetter(expr.substring(1), fullExpr, src);
        return function (obj) { return !getter_2(obj); };
    }
    var dotExpr = bracketsToDots(expr), path = dotExpr.split(".");
    return function (obj) {
...
export function compileSetter(expr, fullExpr, src) {
    if (expr && expr.length) {
        if (expr.charAt(0) === "!") {
            var setter_1 = compileSetter(expr.substring(1), fullExpr, src);
            return function (obj, value) { return setter_1(obj, !value); };
        }
    }
    return dxCompileSetter(expr);
}
